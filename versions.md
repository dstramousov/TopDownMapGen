...
- Микрогеометрия обычных руин получила детерминированные профили разрушения `light`, `moderate` и `heavy`.
- Окончания стен теперь имеют ступенчатые сколы, углы — асимметричное разрушение, а длинные стены могут получать локальные подтайловые проломы.
- После разрушения удаляются изолированные одиночные подтайлы; двери и уже существующие крупные проходы не перекрываются.
- Формат данных остаётся совместимым: `division=4`, sparse `uint16` masks; обновлены версии схем.
- Версия проекта поднята до `0.0.135`.


## v0.0.135 -> v0.0.136

- Крепость разрешена для всех elevation-профилей и при `enabled=true` получает гарантированный fallback размещения.
- Новый публичный конфиг использует `size: small|medium|huge` и `archetype: island|any`; legacy-настройки читаются совместимо.
- Для `island` приоритет размещения: подходящий остров, берег существующего водоёма, затем суша при полном отсутствии воды.
- Размер крепости масштабируется от короткой стороны карты по выбранному профилю; отчёт хранит requested/resolved placement и fallback reason.
- Версия проекта поднята до `0.0.136`.


## v0.0.136 -> v0.0.137

- Для крепости на суше отключён отдельный подъезд: генератор больше не прокладывает бессмысленную дорогу от ворот через карту.
- В отчёте `fortress_approach` фиксируется `status: skipped` и `reason: inland_placement`.
- Общий текстовый отчёт больше не называет сухопутное размещение `lake-island site`.
- Версия проекта поднята до `0.0.137`.


## v0.0.137 -> v0.0.138

- Профили размера крепости сделаны заметно контрастнее: `small=10%`, `medium=16%`, `huge=24%` от короткой стороны карты с прежними безопасными пределами.
...
- Зубцы крепостных стен теперь создаются только на кромке, направленной от центра крепости во внешний мир.
- Внутренняя кромка со стороны двора остаётся плоским боевым ходом без второго ряда зубцов.
- Сохранены текущий ритм зубцов 2/2 и свободные проходы в местах соединения стен с башнями.
- Схема верхней геометрии обновлена до `structure-top-geometry-layer-v5`.


## v0.0.144 -> v0.0.145

- Полностью переписана микрогеометрия внешней оболочки крепости на единой глобальной сетке 4x4.
- Волнистые и прямые стены теперь растеризуются по исходным осевым кривым с постоянной толщиной.
- Башни строятся как независимые точные круговые тела и больше не деформируются примыкающими стенами.
- Стены обрезаются перед башнями, а старое локальное объединение wall/tower masks удалено.
- Верхняя геометрия крепости временно отключена до повторного построения боевого хода и зубцов поверх нового каркаса.


## v0.0.145 -> v0.0.146

- Толщина внешних крепостных стен теперь рассчитывается от диаметра башен, а не фиксируется двумя подтайлами.
- Стена занимает примерно половину диаметра типовой башни и сохраняет эту ширину вдоль прямых и волнистых участков.
- Подготовлена достаточная верхняя площадка для будущих зубцов, наружной кромки и боевого хода.
- Схема микрогеометрии обновлена до `structure-micro-geometry-layer-v10`.


## v0.0.146 -> v0.0.147

- Крепостная стена теперь растеризуется как полоса постоянной ширины по расстоянию до всей осевой полилинии.
- Вычисленная ширина не меняется на прямых, волнах, поворотах и в месте присоединения к башне.
- Все тайлы финальной микрогеометрии стены получают скорректированную высоту, формирующую один плоский абсолютный верх.
- Схемы микрогеометрии и высот обновлены для нового контракта оболочки крепости.


## v0.0.147 -> v0.0.148

- Исправлена валидация VXMAP для крепостной микрогеометрии, расположенной поверх обычного terrain-тайла.
- Высота сооружения теперь проверяется по семантическому `structure_type` и непустой микромаске, а не только по `ruin_wall_blocker`.
- Сохранена строгая проверка: высота допустима только для твёрдых стен, башен и построек; полы и пустые клетки обязаны иметь нулевую высоту.
- Добавлены регрессионные тесты для толстой крепостной стены, выходящей за старый тайловый контур.


## v0.0.148 -> v0.0.149

- Устранены вырезы в круглых башнях, которые возникали, когда старый тайл ворот безусловно пробивал финальную башенную микромаску.
- Воротный проём теперь вырезается только из стены; башня сохраняет цельный круглый корпус.
- Верх каждой связной башни выравнивается по абсолютной высоте с компенсацией рельефа, поэтому на крыше нет ям, ступеней и локально заниженных участков.
- Схемы микрогеометрии и высот обновлены для герметичной оболочки башен.


## v0.0.149 -> v0.0.150

- Завершена базовая геометрия крепости: центральный keep стал цельным массивным корпусом с ровной крышей.
- Для стен, круглых башен и keep снова экспортируется единая плоская верхняя поверхность.
...
- Исправлена поздняя материализация крепости на shore/inland placement: локально поднятый core больше не может выйти за публичный диапазон elevation `-5..20`.
- Runtime elevation, принадлежащий объектам мира, теперь восстанавливается после shaping крепостного острова и подхода, поэтому траншеи, pits и buried bunkers не теряют свой отрицательный footprint при пересечении с зоной крепости.
