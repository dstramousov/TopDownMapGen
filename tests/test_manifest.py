...
def test_build_manifest_marks_primary_and_debug_outputs(tmp_path: Path) -> None:
    ...
    assert manifest["schema_version"] == "generation-manifest-v52"
    assert manifest["versions"]["generator"] == "0.0.142"
    assert manifest["versions"]["schemas"]["manifest"] == "generation-manifest-v52"
