...
def test_build_manifest_marks_primary_and_debug_outputs(tmp_path: Path) -> None:
    ...
    assert manifest["schema_version"] == "generation-manifest-v28"
    assert manifest["versions"]["generator"] == "0.0.41"
    assert manifest["versions"]["schemas"]["manifest"] == "generation-manifest-v28"
