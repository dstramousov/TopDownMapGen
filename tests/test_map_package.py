...
def test_write_map_package_creates_structured_outputs(tmp_path: Path) -> None:
    ...
    assert environment_context["grids"]["road_distance"]["rows"] == [[9, 9], [9, 9]]
    assert environment_context["grids"]["structure_distance"]["rows"] == [[9, 9], [9, 9]]
    ...
    assert structure_top_geometry["schema_version"] == (
        "structure-top-geometry-layer-v7"
    )
