...
`places` — смысловые микролокации: блокпост, бункерная позиция, радиоточка, оборонительная позиция, лесной завал, руинная площадка. Это не отдельные тайлы и не обязательно physical objects. Это группировка объектов, маркеров и зон в маленькую сцену.

С `places-v2` каждое место содержит:

- `bounds` — прямоугольник места в tile-координатах;
- `entrances` — предполагаемые входы в место;
- `danger_level` и `loot_level` — нормализованные gameplay hints `0.0..1.0`;
- `story_role` и `encounter_type` — смысловую роль места;
- `object_refs`, `marker_refs`, `route_refs` — ссылки на связанные сущности;
- `connected_places` — ближайшие связанные места для будущего world graph;
- `biome_tags` — подсказки для визуала, звука, декора и контента.

Игра может использовать `places` для заданий, интересных точек, narrative events, spawn rules и построения графа мира.
